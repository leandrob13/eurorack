...
static inline uint8_t U8Mix(uint8_t a, uint8_t b, uint8_t balance) {
  uint16_t partA = a * (255 - balance);
  uint16_t partB = b * balance;
  uint16_t sum = partA + partB;
  return static_cast<uint8_t>(sum >> 8); // Return the upper byte of the sum.
}

static inline uint8_t U8U8MulShift8(uint8_t a, uint8_t b) {
    // Perform the multiplication and cast the result to a 16-bit integer to
    // hold the full result of the 8-bit multiplication.
    uint16_t temp = static_cast<uint16_t>(a) * static_cast<uint16_t>(b);

    // Shift the result right by 8 bits to get the high byte of the result.
    uint8_t result = static_cast<uint8_t>(temp >> 8);
...
/* static */
void PatternGenerator::LoadSettings(uint8_t byte) {
  options_.unpack(byte);
}

/* static */
uint8_t PatternGenerator::SaveSettings() {
  return options_.pack();
}
...
uint8_t PatternGenerator::swing_amount() {
  if (options_.swing && output_mode() == OUTPUT_MODE_DRUMS) {
    ...
    //uint16_t temp = static_cast<uint16_t>(settings_.options.drums.randomness) * static_cast<uint16_t>(42 + 1);
    //uint8_t value = static_cast<uint8_t>(temp >> 8);
